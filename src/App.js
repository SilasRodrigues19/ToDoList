...
const App = () => {
  ...
  return (
    <>
      <Flex w='100%' h="100vh">
        <Flex w='100%' flexDir='column' ml='20%' mt='5%' mr='20%' color='#fff'>
          <Text fontWeight='700' textAlign='center' mb='10%' fontSize={30}>What's the Plan for Today?</Text>
          <form onSubmit={addTask}>
            <Flex mt='2%'>
              <Input value={newTask} onChange={e => setNewTask(e.target.value)} focusBorderColor='purple.400' variant='outline' w='50%' placeholder='Task to be done' />
              <Button onClick={addTask} ml={5} bg='purple.400' _hover={{ bg: 'purple.300' }} _focus={{ boxShadow: 'none' }}><SmallAddIcon w={5} h={5} /></Button>
            </Flex>
          </form>
          <Tabs mt='2%' w='100%'>
            <TabList >
              <Tab _focus='none' color='purple.200' _selected={{ color: 'purple.400', borderColor: 'purple.400' }}>Incomplete Tasks</Tab>
              <Tab _focus='none' color='purple.200' _selected={{ color: 'purple.400', borderColor: 'purple.400' }}>Completed Tasks</Tab>
            </TabList>
            <TabPanels>
              <TabPanel>
